...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware(['validated'])->group(function () {
        ...
        // ---- PERSETUJUAN DOKUMEN (KHUSUS MANAGER & ADMIN) ----
        Route::middleware('role:super_admin,k3_manager')->group(function () {
            Route::patch('/sops/{sop}/approve-status', [SopController::class, 'approve'])->name('sops.approve-status');
            Route::patch('/sops/{sop}/reject-status', [SopController::class, 'reject'])->name('sops.reject-status');
            Route::patch('/k3-documents/{k3Document}/approve-status', [K3DocumentController::class, 'approve'])->name('k3-documents.approve-status');
        });
