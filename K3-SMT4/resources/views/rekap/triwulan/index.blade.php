...
@section('title', 'Rekap Triwulan')
@section('page-title', 'Rekap Triwulan')
@section('page-subtitle', 'Laporan triwulan SMK3 otomatis')

@section('content')
<div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm">
    <div class="mb-5">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Rekap Triwulan</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400">Pilih tahun dan triwulan untuk melihat rekap.</p>
    </div>
    ...
    <form method="GET" class="grid gap-4 md:grid-cols-3 items-end">
        ...
        <div>
            <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Triwulan</label>
            <select name="quarter" class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                @foreach([1,2,3,4] as $q)
                    <option value="{{ $q }}" {{ $quarter == $q ? 'selected' : '' }}>Triwulan {{ $q }}</option>
                @endforeach
            </select>
        </div>
